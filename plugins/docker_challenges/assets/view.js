...
function get_docker_status(container) {    
    $('#docker_container').html('<div class="text-center"><span>Fetching information... </span><br><i class="fas fa-circle-notch fa-spin fa-1x"></i></div>');
    $.get("/api/v1/docker_status", function(result) {
        ...
        $.each(result['data'], function(i, item) {
            if (item.docker_image == container) {
                ...
                new Promise(resolve => setTimeout(resolve, DELAY_SECONDS * 1000)).then(() => {
                    ...
                    //})
                    $('#docker_container').html('<pre style="color: #e83e8c">Instance Information:<br />' + data + '<div class="mt-2" id="' + String(item.id).substring(0,10) + '_revert_container"></div>');
                    ...
                    var x = setInterval(function() {
                        ...
                        $("#" + String(item.id).substring(0,10) + "_revert_container").html('Next Revert Available in ' + minutes + ':' + seconds);
                        if (distance < 0) {
                            clearInterval(x);
                            $("#" + String(item.id).substring(0,10) + "_revert_container").html('<a onclick="start_container(\'' + item.docker_image + '\');" class=\'btn btn-dark\'><small style=\'color:white;\'><i class="fas fa-redo"></i> Revert</small></a>');
                        }
                    }, 1000);
                    return false;
                })
        ...
        if (!running) {
            $('#docker_container').html('<span><a onclick="start_container(\''+ container + '\');" class="btn btn-dark"><small style="color:white;"><i class="fas fa-play"></i> Start Docker Instance</small></a></span>');
        }
    });
};
...
function start_container(container) {
    if (connection_info === null) {
        connection_info = $('.challenge-connection-info code').text()
    } else {
